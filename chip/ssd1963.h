...
#define   OLIVE        ((128<<16)+(128<<8)+(0))// 橄榄绿：128, 128, 0
#define   LGRAY        ((192<<16)+(192<<8)+(192))// 灰白色：192, 192, 192
#define   DGRAY        ((128<<16)+(128<<8)+(128))// 深灰色：128, 128, 128
#define   BLUE         ((0<<16)+(0<<8)+(255))// 蓝色： 0, 0, 255
#define   GREEN        0x00FF00                	 // 绿色： 0, 255, 0
#define   CYAN         0x00FFFF                    // 青色： 0, 255, 255
#define   RED          0xFF0000                    // 红色： 255, 0, 0
#define   MAGENTA      0xFF00FF                    // 品红： 255, 0, 255
...
#define RGB2BGR_ORDER						(1 << 3)//从frame 缓冲区写到tft面板的时候，自动将RGB转换为BGR格式
#define RGB_NO_CHANGE						(0 << 3)//从frame 缓冲区写到tft面板的时候，RGB格式不变，直接传送过去
...
#define HT_TOTAL 					(DISP_HOR_BACK_PORCH+DISP_HOR_FRONT_PORCH+DISP_HOR_PULSE_WIDTH+_HPS)//显示一行需要的DCLK
...
#define VE_TOTAL 					(DISP_VER_BACK_PORCH+DISP_VER_FRONT_PORCH+DISP_VER_PULSE_WIDTH+_VPS)//显示一帧需要的TH
