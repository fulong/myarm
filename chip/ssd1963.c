...
#define PLL_N	0x02
	...
	LCD_WriteRAM(
	        TFT_PANEL_DATA_WITH_18 + TFT_COLOR_DEPTH_ENHANCEMENT_ENABLE + TFT_FRC_DISABLE
	                + LSHIFT_POLARITY_RISING_EDGE + LLINE_POLARITY_LOW + LFRAME_POLARITY_LOW) ;
	...
	LCD_WriteRAM(SET_GPIO0_VALUE + CLR_GPIO1_VALUE + CLR_GPIO2_VALUE + SET_GPIO3_VALUE) ;                    //GPIO[3:0] out 1

	LCD_WriteCOM(SSD1963_SET_GPIO_CONF) ;
	LCD_WriteRAM(
	        GPIO0_CFG_HOST + GPIO1_CFG_HOST + GPIO2_CFG_HOST + GPIO3_CFG_HOST + GPIO0_OUTPUT
	                + GPIO1_OUTPUT + GPIO2_OUTPUT + GPIO3_INPUT) ;                    //GPIO3=input, GPIO[2:0]=output
	LCD_WriteRAM(GPIO0_NORMAL) ;                    //GPIO0 normal

	LCD_WriteCOM(SSD1963_SET_ADDRESS_MODE) ;                    //rotation
	LCD_WriteRAM(
	        PAGE_ADDRESS_MODE_TOP2BUTTOM + COLUMN_ADDRESS_MODE_L2R + PAGE_COLUMN_ADDRESS_MODE_DISABLE
	                ...
	                RGB_NO_CHANGE
#endif
	                + DISPLAY_LATCH_DATA_DISABLE + FLIP_HORIZONTAL_L2R + FLIP_VERTICAL_T2B) ;
	...
	LCD_WriteRAM(DBC_MANUAL_ENABLE + TRANSITION_EFFECT_DISABLE + DBC_AGGRESSIVE_MODE + DBC_ENABLE) ;
